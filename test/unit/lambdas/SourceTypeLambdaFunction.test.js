...
test('detects type for S3 objects', async () => {
  AWS.mock(
    'S3',
    'getObject',
    Buffer.from(require('fs').readFileSync('test/samples/jpg.jpg')),
  );
...
test('detects type for S3 objects with goofy extensions', async () => {
  AWS.mock(
    'S3',
    'getObject',
    Buffer.from(require('fs').readFileSync('test/samples/jpg.png')),
  );
...
test('does not detects type for undetectable S3 objects', async () => {
  AWS.mock(
    'S3',
    'getObject',
    Buffer.from(require('fs').readFileSync('test/samples/empty')),
  );
