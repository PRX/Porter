...
The `HLS` task provides preset, which determines the specific encoding configuration and parameters that will be used to create the variants. Using presets reduces the amount of per-job configuration that's required, and makes the set of assets resulting from the task more predictable.
