...
Most task configuration is handled using _presets_. The one per-job option is
`AdBreaks` (see below), which conditions the output so ads can be inserted at
exact times.
...
This task is primarily concerned about creating files that are valid for HLS purposes, and less concerned about the HLS-specific details of those files, such as rungs, `BANDWIDTH`, media groups, etc. Those details are determined by the chosen preset, and can be found by inspecting the resulting playlist files. The task does report some minimal details about the files being created and delivered to the chosen destination, so that apps can track relevant files, but the task results should not be expected to include comprehensive HLS-specific information.
...
Individual variant details in the task output will include the appropriate label, so an application receiving the callback can easily extract information about a specific, expected variant's assets.

- **Standard Podcast 2026 v1:** A 480p/720p/1080p H.264 ladder in MPEG-TS, with an
  audio rendition and an I-frame-only variant for scrubbing.
  Video and audio are delivered separately (demuxed): the video variants carry no
  audio, and reference one audio rendition that is also published as a standalone audio-only variant.
  Possible labels: `AUDIO`, `480P`, `720P`, `1080P`, `IFRAME`.

  Video rungs taller than the source are skipped rather than upscaled, so a job
  may produce fewer than all of them; the skipped rungs are listed in the task's logs.

  Two master playlists are produced: `index.m3u8` is the default w/an audio-only variant,
  and`index-compat.m3u8` is identical but omits it.
  The `compat`ibility playlist is for players whose adaptive logic treats the audio-only variant,
  if present, as the bottom rung of the video ladder and plays video w/o audio (e.g. VLC).

#### Ad Breaks

Ads can only be inserted into an HLS stream *between* segments
(i.e. required for discontinuity based SSAI, but also preferred for interstitials).
The optional `AdBreaks` property lists times, in seconds from the start of the asset, where
breaks are expected. The task will then create a segment layout that puts a boundary
and a keyframe at each one, in every video variant and in the audio rendition.

**WARNING: The task fails rather than delivering a break it could not honor.**

These are hard errors, each naming the break at fault:
- **Two breaks landing on the same video frame.** Below one frame they cannot be
  told apart.
- **A break outside the asset**, including one that rounds up past the end.
- **A break too early to be reachable.** The `ffmpeg hls` muxer's split threshold is cumulative,
  so the n-th boundary cannot precede `n x HLS_TIME`. This only bites near the
  *start* of the asset — once the stream is ahead of the running threshold it
  stops applying, which is why closely spaced boundaries are fine mid-stream but
  not at the beginning.

Breaks closer together than the preset's minimum segment length are *not* an
error. That minimum is a quality floor on the regular grid, not a limit of the
format: it evicts grid points near a break, but never rejects a break. Both
muxers cut boundaries closer together than it.

```json
{ "AdBreaks": [30.0, 90.5] }
```

The `AdBreak`s requested may not exactly match the timing of video frames.
As such, break times are quantized to whole video frames:
e.g. a request for `90.47` at 30fps lands on `90.466667`.
The task result includes where each break _actually_ landed:
...
```json
{
    "AdBreaks": [
        {
            "RequestedTime": 90.47,
            "ActualTime": 90.466667,
            "InsertAfterSegmentIndex": 14
        }
    ]
}
```

`RequestedTime` is echoed back exactly as given, so entries can be matched to the
request without recomputing anything. `ActualTime` is the boundary you can splice
at, measured from the playlist: the request snapped to a whole video frame, since
video cannot be cut mid-frame. The two differ by at most half a frame — a larger
gap would mean the keyframe list was ignored, and the task fails rather than
reporting that. `InsertAfterSegmentIndex`
is the zero-based index of the playlist entry to insert after: to splice, add an
`EXT-X-DISCONTINUITY` and the ad segment URIs immediately after that entry in each
media playlist. The surrounding entries do not change.

`AdBreaks` is omitted from the results when no breaks were requested, in which
case segments fall on a regular grid.
...
```json
{
    ...
    "Assets": {
        ...
        "CompatibilityMasterPlaylist": {
            "ObjectKey": "myPrefix/index-compat.m3u8"
        },
        "Variants": [
            {
                "PresetLabel": "AUDIO",
