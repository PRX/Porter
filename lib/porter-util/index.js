...
// Get the path to a binary based on environment variables,
// either from the BIN_DIR env var, or from a specific binary path override
// e.g. FFMPEG_PATH
export function binDir(bin) {
  let path = null;
  const bin_path_var = `${bin.toUpperCase()}_PATH`;
  if (process.env[bin_path_var]) {
    path = process.env[bin_path_var];
  } else {
    path = process.env.BIN_DIR || "/opt/bin";
    path = pathJoin(path, bin);
  }
