...
def send_to_s3(filename)
  ...
  role = sts.assume_role({
    ...
    role_session_name: "porter_hls_parallel_task"
  })
  ...
  put_object_params[:key] = [destination["ObjectKeyPrefix"], filename].join
  ...
  puts JSON.dump({
    msg: "Writing #{filename} to S3 destination",
    filename: filename,
    bucket_region: bucket_region,
    bucket: put_object_params[:bucket],
    object_key: put_object_params[:key]
  })
  put_ouput_s3tm = Aws::S3::TransferManager.new(client: s3_writer)
  put_ouput_s3tm.upload_file(filename, **put_object_params)

  {
    ObjectKey: put_object_params[:key]
  }
end
